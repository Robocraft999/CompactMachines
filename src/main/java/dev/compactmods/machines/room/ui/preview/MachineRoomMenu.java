    ...
    public static MenuProvider provider(MinecraftServer server, RoomInstance roomInstance) {
        return new MenuProvider() {
            ...
            public AbstractContainerMenu createMenu(int winId, Inventory inv, Player player) {
                ...
                AtomicReference<GlobalPos> viewing = new AtomicReference<>();
                player.getCapability(Rooms.DataAttachments.PLAYER_ROOM_DATA).ifPresent(cap -> viewing.set(cap.getOpenMachinePos()));
                return new MachineRoomMenu(winId, roomInstance.code(), viewing.get(), "Room Preview");
            }
        };
    }
